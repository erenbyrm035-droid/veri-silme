...
import type { ExerciseVersion } from "@/lib/database.types";
...
export function VersionHistory({
  ...
  function restore(versionId: string) {
    ...
    startTransition(async () => {
      const res = await restoreVersion({ exerciseId, versionId });
  ...
  return (
    <Card className="space-y-4 p-5">
      <h3 className="flex items-center gap-2 text-sm font-semibold">
        <History size={16} className="text-brand" /> Sürüm Geçmişi
      </h3>
      {error && <p className="text-sm text-coral">{error}</p>}
      {versions.length === 0 ? (
        <p className="text-sm text-fg-muted">Henüz kayıtlı sürüm yok. Kaydettikçe otomatik oluşur.</p>
      ) : (
        <ol className="space-y-2">
          {versions.map((v) => {
            const snap = v.snapshot as { name?: string };
            return (
              <li key={v.id} className="flex items-center justify-between gap-3 rounded-xl border border-ink-border bg-ink-soft/40 px-3 py-2">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-fg-muted">
                    {v.changed_by_name ?? "Bilinmiyor"} · {fmt(v.created_at)}
                    {v.change_note ? ` · ${v.change_note}` : ""}
                  </p>
                </div>
                <Button size="sm" variant="outline" onClick={() => setRestoreId(v.id)} disabled={isPending}>
      ...
      <ConfirmDialog
        ...
        description="Mevcut içerik yeni bir yedek olarak saklanır, seçili sürüm geri yüklenir."
