...
import { searchProgramsAction } from "../search-action";
import type { ProgramRelationType } from "@/lib/database.types";
import type { RelationWithTarget } from "../queries";

export function RelationManager({ programId, relations }: { programId: string; relations: RelationWithTarget[] }) {
  const router = useRouter();
  const [relation, setRelation] = React.useState<ProgramRelationType>("similar");
  ...
  React.useEffect(() => {
    ...
    const id = setTimeout(async () => setResults(await searchProgramsAction(term, programId)), 300);
  ...
  function add(relatedId: string) {
    ...
    startTransition(async () => {
      const res = await addRelation({ programId, relatedId, relation });
      if (!res.ok) return setError(res.error ?? "Eklenemedi.");
      setTerm(""); setResults([]); router.refresh();
    });
  }
  function remove(id: string) {
    startTransition(async () => { await removeRelation({ id, programId }); router.refresh(); });
  }

  const grouped = RELATION_VALUES.map((rel) => ({ rel, items: relations.filter((r) => r.relation === rel) })).filter((g) => g.items.length > 0);

  return (
    <Card className="space-y-4 p-5">
      <h3 className="flex items-center gap-2 text-sm font-semibold"><Link2 size={16} className="text-brand" /> İlişkili Programlar</h3>
      <div className="flex flex-wrap gap-2">
        <div className="w-44">
          <Select value={relation} onChange={(e) => setRelation(e.target.value as ProgramRelationType)}>
        ...
        <div className="relative min-w-[200px] flex-1">
          ...
          <Input value={term} onChange={(e) => setTerm(e.target.value)} placeholder="Program ara…" className="pl-9" />
          {results.length > 0 && (
      ...
      <div className="space-y-3">
        ...
        {grouped.map((g) => (
          <div key={g.rel}>
            <p className="mb-1.5 text-xs font-semibold text-fg-muted">{RELATION_LABELS[g.rel]} <Badge variant="secondary">{g.items.length}</Badge></p>
